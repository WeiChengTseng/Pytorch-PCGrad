...
def normal_error(x_pred, x_output):
    ...
    return np.mean(error), np.median(error), np.mean(error < 11.25), np.mean(
        error < 22.5), np.mean(error < 30)
...
def multi_task_trainer(train_loader,
                       test_loader,
                       multi_task_model,
                       device,
                       optimizer,
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            if opt.weight == 'equal' or opt.weight == 'dwa':
                loss = sum([
                    lambda_weight[i, index] * train_loss[i] for i in range(3)
                ])
            else:
                loss = sum(1 / (2 * torch.exp(logsigma[i])) * train_loss[i] +
                           logsigma[i] / 2 for i in range(3))
        ...
        print(
            ...
            .format(index, avg_cost[index, 0], avg_cost[index,
                                                        1], avg_cost[index, 2],
                    avg_cost[index, 3], avg_cost[index, 4], avg_cost[index, 5],
                    avg_cost[index, 6], avg_cost[index, 7], avg_cost[index, 8],
                    avg_cost[index, 9], avg_cost[index, 10], avg_cost[index,
                                                                      11],
                    avg_cost[index, 12], avg_cost[index, 13], avg_cost[index,
                                                                       14],
                    avg_cost[index, 15], avg_cost[index, 16], avg_cost[index,
                                                                       17],
                    avg_cost[index, 18], avg_cost[index, 19], avg_cost[index,
                                                                       20],
                    avg_cost[index, 21], avg_cost[index, 22], avg_cost[index,
                                                                       23]))
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            if opt.task == 'semantic':
                ...
                conf_mat.update(
                    train_pred.argmax(1).flatten(), train_label.flatten())
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                ...
                if opt.task == 'semantic':
                    ...
                    conf_mat.update(
                        test_pred.argmax(1).flatten(), test_label.flatten())
                ...
                if opt.task == 'normal':
                    ...
                    cost[19], cost[20], cost[21], cost[22], cost[
                        23] = normal_error(test_pred, test_normal)
        ...
        if opt.task == 'semantic':
            print(
                'Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} TEST: {:.4f} {:.4f} {:.4f}'
                .format(index, avg_cost[index, 0], avg_cost[index, 1],
                        avg_cost[index, 2], avg_cost[index, 12],
                        avg_cost[index, 13], avg_cost[index, 14]))
        if opt.task == 'depth':
            print(
                'Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} TEST: {:.4f} {:.4f} {:.4f}'
                .format(index, avg_cost[index, 3], avg_cost[index, 4],
                        avg_cost[index, 5], avg_cost[index, 15],
                        avg_cost[index, 16], avg_cost[index, 17]))
        if opt.task == 'normal':
            print(
                'Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} TEST: {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f}'
                .format(index, avg_cost[index, 6], avg_cost[index, 7],
                        avg_cost[index, 8], avg_cost[index, 9],
                        avg_cost[index, 10], avg_cost[index,
                                                      11], avg_cost[index, 18],
                        avg_cost[index, 19], avg_cost[index,
                                                      20], avg_cost[index, 21],
